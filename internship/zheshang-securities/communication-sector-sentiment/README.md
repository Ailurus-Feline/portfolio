# Communication Sector Sentiment Index

A quantitative sentiment indicator for the Shenwan Communication sector (index code: **801770**), built from market-breadth signals across constituent stocks and visualized against the log-transformed sector index.
...
**Universe:** ~123 Shenwan Communication constituent stocks  
**History:** Daily closes from 2007 onward  
**Benchmark:** Shenwan Communication Index (801770)
...
Regime labels are projected onto the **log-transformed** Shenwan Communication Index (`ln(close)`) for visual comparison.
...
```
communication-sector-sentiment/
├── README.md
├── requirements.txt
├── .gitignore
├── data/
...
|---------|--------|-------|
| Constituent daily prices | `date`, `symbol`, `close` | 123 stocks, from 2007 |
| Sector index | `date`, `close` | 801770 |
| Market capitalization | `date`, `symbol`, `market_cap` | For daily top-3 ranking |
| Stock metadata (optional) | `symbol`, `board` | For limit-up threshold (10% vs 20%) |
...
```bash
cd communication-sector-sentiment
python -m venv .venv
...
- **Left Y-axis:** `ln(Shenwan Communication Index)`
...
- [ ] Phase 0: Repo setup (`requirements.txt`, `.gitignore`, directory layout)
